...
export function useCEP(options: UseCEPOptions = {}): UseCEPReturn {
  ...
  const limparCEP = useCallback((cep: string): string => cep.replace(/\D/g, ''), [])
  ...
  const buscarCEPInterno = useCallback(async (cepLimpo: string) => {
    ...
    try {
      ...
      if (err instanceof Error) {
        ...
        const errorMessage = err.message === 'CEP não encontrado' 
