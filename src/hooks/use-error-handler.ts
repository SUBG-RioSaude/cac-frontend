...
export function useErrorHandler() {
  ...
  const handleError = useCallback((error: ErrorInfo | Error | string, statusCode?: number) => {
    ...
    navigate(redirectPath, {
      state: { 
        error: errorInfo.message || errorInfo.details || 'Erro não especificado',
        fullError: errorInfo 
      }
    })
  }, [navigate])

  const handleHttpError = useCallback((response: Response, customMessage?: string) => {
    const errorInfo: ErrorInfo = {
      code: response.status,
      message: customMessage || `Erro HTTP ${response.status}: ${response.statusText}`,
      url: response.url,
      timestamp: new Date()
    }
  ...
  const handleApiError = useCallback((error: unknown, fallbackCode = 500) => {
    ...
    if (error && typeof error === 'object' && 'response' in error) {
      ...
      const errorInfo: ErrorInfo = {
        code: status,
        message: (data && typeof data === 'object' && ('message' in data || 'error' in data)) 
      ...
      // Outros erros
      handleError(error, fallbackCode)
    }
  }, [handleError])
