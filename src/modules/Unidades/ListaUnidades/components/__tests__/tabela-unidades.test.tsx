...
describe('TabelaUnidades', () => {
  ...
  it('deve renderizar todas as unidades na tabela desktop', () => {
    ...
    expect(
      screen.getAllByText('Hospital Municipal de Saude Sao Joao'),
    ).toHaveLength(2) // mobile + desktop
    expect(
      screen.getAllByText('Centro de Atencao Psicossocial Norte'),
    ).toHaveLength(2)
  ...
  it('deve chamar onOrdenacao ao clicar nos cabeçalhos', async () => {
    ...
    renderWithRouter(
      <TabelaUnidades {...defaultProps} onOrdenacao={mockOnOrdenacao} />,
    )
  ...
  it('deve exibir informações de paginação', () => {
    ...
    expect(
      screen.getByText(/Mostrando 1 a 3 de 3 unidades/),
    ).toBeInTheDocument()
  })
  ...
  it('deve renderizar texto longo nos campos apropriados', () => {
    ...
    expect(nomeElementDesktop).toHaveClass(
      'text-sm',
      'font-medium',
      'leading-relaxed',
    )
