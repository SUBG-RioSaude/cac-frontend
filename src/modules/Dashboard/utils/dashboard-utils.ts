 ...
 */
export const createDashboardMetric = (
  atual: number,
  anterior: number,
): DashboardMetric => {
 ...
 */
export const hasExpiredOrExpiringDocs = (
 ...
 */
export const hasPublicationPendencies = (
  contrato: ContratoDetalhado,
): boolean => {
...
export const isExpiringSoon = (
  contrato: ContratoDetalhado,
): { isExpiring: boolean; days: number } => {
  const now = new Date()
  const endDate = parseISO(contrato.dataTermino)
  const daysUntilExpiry = Math.ceil(
    (endDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24),
 ...
 */
export const classifyContractRisk = (
  contrato: ContratoDetalhado,
): RiskLevel => {
  // Alto Risco: Docs vencidos/vencendo, atrasos, pendências de publicação
  if (
    hasExpiredOrExpiringDocs(contrato) ||
...
export const hasActiveFilters = (filters: DashboardFilters): boolean => {
  ...
  const isCurrentPeriod =
    filters.periodo.mes === today.getMonth() + 1 &&
...
export const getActivityConfig = (type: ActivityType) => {
  ...
  return configs[type] || { icon: 'Edit', color: 'text-yellow-600', bg: 'bg-yellow-100' }
}
