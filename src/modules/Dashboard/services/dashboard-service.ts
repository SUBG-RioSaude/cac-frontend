...
import { getContratos, getContratosVencendo, getContratosVencidos } from '@/modules/Contratos/services/contratos-service'
...
import type { Contrato, ContratoDetalhado, ContratoStatus } from '@/modules/Contratos/types/contrato'
import { 
...
import { subMonths, format, parseISO, startOfMonth, endOfMonth, isAfter, isBefore } from 'date-fns'
 ...
 */
const filterContractsByPeriod = (contratos: Contrato[], filters: DashboardFilters): Contrato[] => {
  const startDate = startOfMonth(new Date(filters.periodo.ano, filters.periodo.mes - 1))
  const endDate = endOfMonth(new Date(filters.periodo.ano, filters.periodo.mes - 1))
 ...
 */
const getPreviousPeriodData = async (filters: DashboardFilters): Promise<Contrato[]> => {
  const previousMonth = subMonths(new Date(filters.periodo.ano, filters.periodo.mes - 1), 1)
...
export const fetchDashboardMetrics = async (filters: DashboardFilters): Promise<DashboardMetrics> => {
  try {
    ...
    const filtrosBaseGeral = {
      unidadeSaudeId: filters.unidades.length === 1 ? filters.unidades[0] : undefined,
      tamanhoPagina: 1000
    }
    ...
    const [
      ...
      getContratos({
        ...filtrosBaseGeral,
        filtroStatus: filters.status.length > 0 ? filters.status.join(',') : undefined
      }),
    ...
    const filteredCurrentContracts = filterContractsByPeriod(currentContracts, filters)
    ...
    const contratosAtivos = filteredCurrentContracts.filter(c => c.status === 'ativo').length
    const valorTotal = filteredCurrentContracts.reduce((sum, c) => sum + (c.valorGlobal || 0), 0)
    ...
    try {
      ...
      console.error('Erro ao buscar contratos vencendo do período anterior:', error)
      contratosVencendoAnterior = 0
    }
    
    // Calcular métricas do período anterior
    const totalContratosAnterior = previousContracts.length
    const contratosAtivosAnterior = previousContracts.filter(c => c.status === 'ativo').length
    const valorTotalAnterior = previousContracts.reduce((sum, c) => sum + (c.valorGlobal || 0), 0)
    
    return {
      totalContratos: createDashboardMetric(totalContratos, totalContratosAnterior),
      contratosAtivos: createDashboardMetric(contratosAtivos, contratosAtivosAnterior),
      contratosVencendo: createDashboardMetric(contratosVencendo, contratosVencendoAnterior),
 ...
 */
export const fetchStatusDistribution = async (filters: DashboardFilters): Promise<StatusDistributionData[]> => {
  try {
    ...
    filteredContracts.forEach(contract => {
      if (contract.status && contract.status in statusCounts && 
          contract.status !== 'vencendo' && contract.status !== 'vencido') {
    ...
    return result.length > 0 ? result : [
      { name: 'Nenhum contrato', value: 0, color: 'hsl(var(--muted))', percentage: 0 }
    ]
 ...
 */
export const fetchStatusTrend = async (filters: DashboardFilters): Promise<StatusTrendData[]> => {
  try {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      try {
        ...
        months.push({
          ...
          pendentes: contracts.filter(c => c.status === 'em_aprovacao' || c.status === 'rascunho').length,
 ...
 */
export const fetchTypeDistribution = async (filters: DashboardFilters): Promise<TypeDistributionData[]> => {
  try {
    ...
    const result: TypeDistributionData[] = Object.entries(typeCounts).map(([tipo, data]) => ({
 ...
 */
export const fetchRecentContracts = async (filters: DashboardFilters): Promise<RecentContract[]> => {
  try {
    ...
    return sortedContracts.map(contract => ({
      ...
      objeto: contract.objeto || contract.descricaoObjeto || 'Objeto não informado',
...
export const fetchRecentActivities = async (): Promise<DashboardActivity[]> => {
  try {
    ...
    const activities: DashboardActivity[] = contracts.map((contract, index) => {
      const tipos: Array<'cadastrado' | 'aprovado' | 'atualizado' | 'cancelado' | 'renovado'> = ['cadastrado', 'aprovado', 'atualizado']
 ...
 */
export const fetchRiskAnalysis = async (filters: DashboardFilters): Promise<DashboardRisks> => {
  try {
    // Construir filtros base
    const filtrosBaseGeral = {
      unidadeSaudeId: filters.unidades.length === 1 ? filters.unidades[0] : undefined,
    ...
    contratosVencidos.forEach(contrato => {
      riskContracts.alto.push({
        ...
        objeto: contrato.objeto || contrato.descricaoObjeto || 'Objeto não informado',
        risco: 'alto',
    ...
    filteredContracts.forEach(contrato => {
      ...
      if (!isVencido && !isVencendo && contrato.status === 'ativo') {
        ...
        const riskLevel = classifyContractRisk(contratoDetalhado as ContratoDetalhado)
    ...
    const totalContratos = riskContracts.alto.length + riskContracts.medio.length + riskContracts.baixo.length
...
export const fetchDashboardData = async (filters: DashboardFilters): Promise<DashboardData> => {
  try {
    ...
    throw new Error('Falha ao carregar dados do dashboard. Contate o administrador do sistema.')
  }
}
