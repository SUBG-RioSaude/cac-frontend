/**
 * TrendSection - Gráfico de Tendência dos Contratos - OTIMIZADO
 *
...
import type { DashboardFilters, StatusTrendData } from '../../types/dashboard'
...
const TrendSectionComponent = ({
  ...
  // Formatar período para exibir no título (Global ou mês/ano)
  const periodoFormatado = useMemo(() => {
    if (!filters) return ''

    // Se visualização global, retorna "Global"
    if (filters.tipoVisualizacao === 'global') {
      return 'Global'
    }

    // Se visualização por período, formata mês/ano
    const meses = [
      'janeiro',
      'fevereiro',
      'março',
    ...
    const mesNome = meses[filters.periodo.mes - 1] || ''
    return `${mesNome}/${filters.periodo.ano}`
  }, [filters])

  if (isLoading) {
    return (
      <Card className="h-[590px] w-full">
        <CardHeader>
  ...
  return (
    <Card className="flex h-[590px] w-full flex-col">
...
export const TrendSection = memo(TrendSectionComponent, (prev, next) => {
  // Comparar dados e filtros para decidir se deve re-renderizar
  return (
