...
export const TIMELINE_ENTRIES_MOCK: TimelineEntry[] = [
  {
    ...
    descricao: 'Aprovado acréscimo de 15% no valor original do contrato devido à necessidade de serviços adicionais de manutenção preventiva no sistema de climatização.',
    dataEvento: '2024-08-15T10:30:00Z',
    autor: {
      id: '101',
      nome: 'Maria Santos',
      avatar: 'https://images.unsplash.com/photo-1494790108755-2616b612-0000-a76c-3db3?w=32&h=32&fit=crop&crop=face',
      tipo: 'gestor'
    },
    status: 'ativo',
    prioridade: 'alta',
  ...
  },
  {
    ...
    descricao: 'Marco automático registrando o início oficial da vigência contratual conforme assinatura das partes.',
  ...
  },
  {
    ...
    descricao: 'Realizada reunião presencial para alinhamento de expectativas e esclarecimento de dúvidas sobre o escopo dos serviços. Participaram: equipe técnica, gestor do contrato e representantes do fornecedor.',
  ...
  },
  {
    ...
    descricao: 'Solicitada e aprovada prorrogação de 6 meses no prazo de vigência devido a atrasos na liberação de áreas para execução dos serviços.',
  ...
  },
  {
    ...
    descricao: 'Adicionado relatório detalhado de medição dos serviços executados no mês de julho/2024, incluindo fotos e planilhas de acompanhamento.',
  ...
  },
  {
    ...
    descricao: 'Lembrete: O fornecedor deve entregar o relatório trimestral de desempenho até o dia 30/09/2024, conforme cláusula 15.3 do contrato.',
  ...
  },
  {
    ...
    descricao: 'Identificada não conformidade relacionada ao atraso de 3 dias na execução da manutenção preventiva do sistema de ar condicionado. Fornecedor foi notificado e apresentou justificativa.',
  ...
  },
  {
    ...
    descricao: 'Marco automático indicando que o contrato atingiu 50% do seu prazo total de vigência. Sugerida avaliação de desempenho para planejamento de renovação.',
  ...
  },
  {
    ...
    descricao: 'Realizada avaliação semestral do desempenho do fornecedor. Resultado: SATISFATÓRIO (8.5/10). Destaques: pontualidade, qualidade técnica e relacionamento. Pontos de melhoria: comunicação proativa.',
  ...
  },
  {
    ...
    descricao: 'Em análise: proposta de inclusão de serviços de jardinagem nas áreas externas. Valor estimado: R$ 12.000,00 adicionais. Aguardando aprovação da diretoria.',
...
export const TIMELINE_STATS_MOCK = {
  ...
  alteracoesContratuais: TIMELINE_ENTRIES_MOCK.filter(e => e.tipo === 'alteracao_contratual').length,
  observacoes: TIMELINE_ENTRIES_MOCK.filter(e => e.categoria === 'observacao').length,
  milestones: TIMELINE_ENTRIES_MOCK.filter(e => e.categoria === 'milestone').length,
  ...
  proximosPrazos: TIMELINE_ENTRIES_MOCK.filter(e => 
    e.milestone?.dataLimite && !e.milestone.concluido &&
    new Date(e.milestone.dataLimite) > new Date()
  ).sort((a, b) => 
    new Date(a.milestone!.dataLimite!).getTime() - new Date(b.milestone!.dataLimite!).getTime()
  )
}