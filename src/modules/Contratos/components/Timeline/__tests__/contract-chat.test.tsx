...
describe('ContractChat', () => {
  ...
  describe('Renderização inicial', () => {
    it('deve renderizar header com título e informações do contrato', () => {
      ...
      expect(screen.getByText(/Registre observações e acompanhamento/)).toBeInTheDocument()
    })
    ...
    it('deve renderizar área de input para nova observação', () => {
      ...
      expect(screen.getByPlaceholderText(/Registre uma observação sobre o contrato/)).toBeInTheDocument()
  ...
  describe('Exibição de mensagens', () => {
    it('deve renderizar mensagens do chat', () => {
      ...
      expect(screen.getByText('Preciso verificar o cronograma de entregas com o fornecedor.')).toBeInTheDocument()
      expect(screen.getByText('Maria Santos foi designada como gestora do contrato')).toBeInTheDocument()
    })
    ...
    it('deve distinguir visualmente mensagens do sistema', () => {
      ...
      const mensagemSistema = screen.getByText('Maria Santos foi designada como gestora do contrato')
  ...
  describe('Botão de marcar como alteração', () => {
    it('deve mostrar botão ao passar mouse sobre mensagem de usuário', async () => {
      ...
      const mensagem = screen.getByText('Preciso verificar o cronograma de entregas com o fornecedor.')
      ...
      if (container) {
        ...
        const botaoMarcar = container.querySelector('button[title="Marcar como alteração contratual"]')
    ...
    it('deve chamar callback ao clicar no botão de marcar', async () => {
      ...
      const mensagemContainer = screen.getByText('Preciso verificar o cronograma de entregas com o fornecedor.').closest('.group')
    ...
    it('não deve mostrar botão para mensagens do sistema', () => {
      ...
      expect(container?.querySelector('button[title="Marcar como alteração contratual"]')).not.toBeInTheDocument()
    })
  })
  ...
  describe('Envio de nova observação', () => {
    it('deve permitir digitar nova observação', async () => {
      ...
      const textarea = screen.getByPlaceholderText(/Registre uma observação sobre o contrato/)
      
      // Usar fireEvent que é mais direto
      fireEvent.change(textarea, { target: { value: 'Nova observação sobre o contrato' } })
    ...
    it('deve enviar mensagem ao clicar em registrar', async () => {
      ...
      // Digitar texto
      fireEvent.change(textarea, { target: { value: 'Nova observação importante' } })
      ...
      await waitFor(() => {
        expect(screen.getByText('Nova observação importante')).toBeInTheDocument()
      })
    })
  ...
  describe('Dicas e instruções', () => {
    it('deve mostrar dicas de uso do componente', () => {
      ...
      expect(screen.getByText(/Use este espaço para registrar observações importantes/)).toBeInTheDocument()
      expect(screen.getByText(/Passe o mouse sobre observações de outros usuários/)).toBeInTheDocument()
    })
  ...
  describe('Responsividade e UX', () => {
    it('deve aplicar classes responsivas corretas', () => {
      ...
      const card = screen.getByText('Observações do Contrato').closest('.flex.h-full')
