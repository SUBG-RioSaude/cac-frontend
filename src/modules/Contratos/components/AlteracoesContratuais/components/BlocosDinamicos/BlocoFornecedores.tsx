...
function transformSupplierApiData(supplier: FornecedorResumoApi): TransformedFornecedor {
  return {
    id: supplier.id,
    cnpj: supplier.cnpj,
  ...
  const fornecedoresDesvinculadosCompletos = useMemo(() => {
    return (
      dados.fornecedoresDesvinculados
        ?.map((id: string) => fornecedoresApi.find((f: TransformedFornecedor) => f.id === id))
    ...
    <div className="space-y-6">
      ...
      {fornecedoresVinculadosCompletos.length > 0 && (
        <Card className="border-green-200">
          ...
          <CardContent className="space-y-4">
            {fornecedoresVinculadosCompletos.map(
              (fv: FornecedorVinculado) => (
                <div
                  key={fv.empresaId}
