...
export function BlocoClausulas({
  ...
  const handleFieldChange = useCallback((field: keyof IBlocoClausulas, value: string) => {
    const novosDados = {
      ...dados,
  ...
  const inserirTemplate = useCallback((field: keyof IBlocoClausulas, template: string) => {
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Badge variant={clausulasPreenchidas > 0 ? 'default' : 'secondary'}>
            {clausulasPreenchidas} campo{clausulasPreenchidas !== 1 ? 's' : ''} preenchido{clausulasPreenchidas !== 1 ? 's' : ''}
          </Badge>
        ...
        <Card className="bg-purple-50 border-purple-200">
          <CardContent className="pt-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div>
                <Label className="text-xs text-purple-600">Número do Contrato</Label>
            ...
            {contractInfo.objeto && (
              <div className="mt-3 pt-3 border-t border-purple-200">
                <Label className="text-xs text-purple-600">Objeto do Contrato</Label>
                <p className="text-sm text-purple-800 mt-1 line-clamp-2">
                  {contractInfo.objeto}
                </p>
              </div>
            )}
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  onChange={(e) => handleFieldChange('clausulasExcluidas', e.target.value)}
                  disabled={disabled}
                  rows={4}
                  className={cn(
                    'min-h-[100px]',
              ...
              {/* Templates */}
              <div className="space-y-2">
                <Label className="text-xs text-gray-500">Exemplos rápidos:</Label>
                <div className="flex flex-wrap gap-2">
                  {templates.excluidas.map((template, index) => (
                    <Button
                      ...
                      variant="outline"
                      size="sm"
                      onClick={() => inserirTemplate('clausulasExcluidas', template)}
                      disabled={disabled}
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  onChange={(e) => handleFieldChange('clausulasIncluidas', e.target.value)}
                  disabled={disabled}
                  rows={6}
                  className={cn(
                    'min-h-[120px]',
                ...
                <div className="flex flex-wrap gap-2">
                  {templates.incluidas.map((template, index) => (
                    <Button
                      ...
                      variant="outline"
                      size="sm"
                      onClick={() => inserirTemplate('clausulasIncluidas', template)}
                      disabled={disabled}
                      className="text-xs h-auto py-1 px-2"
                    >
                      Modelo {index + 1}
        ...
        <TabsContent value="alteradas" className="space-y-4">
          <Card className="border-blue-200">
            <CardHeader className="pb-3">
              ...
              <p className="text-sm text-gray-600">
                Especifique as cláusulas existentes que terão seu conteúdo modificado
              </p>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  onChange={(e) => handleFieldChange('clausulasAlteradas', e.target.value)}
                  disabled={disabled}
                  rows={5}
                  className={cn(
                    'min-h-[110px]',
                ...
                <div className="flex flex-wrap gap-2">
                  {templates.alteradas.map((template, index) => (
                    <Button
                      ...
                      variant="outline"
                      size="sm"
                      onClick={() => inserirTemplate('clausulasAlteradas', template)}
                      disabled={disabled}
                      className="text-xs h-auto py-1 px-2"
                    >
                      Modelo {index + 1}
      ...
      <div className="bg-blue-50 p-4 rounded-lg">
        <div className="flex items-start gap-3">
          ...
          <div className="text-sm text-blue-800">
            <h4 className="font-medium mb-1">Dicas para redação de cláusulas:</h4>
            <ul className="space-y-1 text-blue-700">
              ...
              <li>• Para alterações, cite claramente o texto original e o novo</li>
