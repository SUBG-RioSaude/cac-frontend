...
describe('RegistroAlteracoes', () => {
  ...
  it('deve ordenar as alterações por data (mais recente primeiro)', () => {
    ...
    const alteracoes = screen.getAllByText(/Criação do Contrato|Designação de Fiscais|Primeiro Pagamento|Atualização de Documentos|Alteração de Valor|Prorrogação de Prazo/)
  ...
  it('deve exibir cores corretas para cada tipo de alteração', () => {
    ...
    // 6 alterações na linha do tempo (h3) + 4 etapas fixas (h4) = 10 elementos
    const containersIcones = screen.getAllByText(/Criação do Contrato|Designação de Fiscais|Primeiro Pagamento|Atualização de Documentos|Alteração de Valor|Prorrogação de Prazo/)
  ...
  it('deve exibir descrições das alterações', () => {
    ...
    expect(screen.getByText('Fiscais administrativos designados: Maria Silva Santos e João Carlos Oliveira')).toBeInTheDocument()
    expect(screen.getByText('Primeiro pagamento realizado no valor de R$ 104.166,67')).toBeInTheDocument()
  })
  ...
  it('deve exibir a linha vertical do tempo', () => {
    ...
    const linhaVertical = document.querySelector('.absolute.left-6.top-0.bottom-0.w-0\\.5.bg-border')
  ...
  it('deve exibir detalhes das alterações quando disponíveis', () => {
    ...
    expect(screen.getByText(/Contrato criado no sistema/)).toBeInTheDocument()
    expect(screen.getByText(/Maria Silva Santos e João Carlos Oliveira/)).toBeInTheDocument()
  ...
  it('deve exibir as etapas do contrato com status correto', () => {
    ...
    const etapas = screen.getAllByText(/Criação do Contrato|Designação de Fiscais|Primeiro Pagamento|Atualização de Documentos/)
  ...
  it('deve aplicar classes CSS corretas para responsividade', () => {
    ...
    const containerResponsivo = document.querySelector('.grid.grid-cols-1.sm\\:grid-cols-2.lg\\:grid-cols-4')
