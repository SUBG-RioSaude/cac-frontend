...
export const TabEmpenhos = ({
  ...
  const temEmpenhosIniciais = useMemo(
    () => empenhosIniciais.length > 0,
    [empenhosIniciais.length],
  )

  // Estado local para empenhos quando temos dados iniciais
  const [empenhosLocais, setEmpenhosLocais] =
    useState<Empenho[]>(empenhosIniciais)
  ...
  const hookEnabled = useMemo(
    () => !temEmpenhosIniciais && !!contratoId,
    [temEmpenhosIniciais, contratoId],
  )
  const {
    empenhos,
  ...
  const { nomesUnidades, carregando: carregandoNomes } =
    useUnidadesNomes(unidadesVinculadas)
  ...
  const empenhosExibidos = useMemo(() => {
    ...
    if (filtroUnidade && filtroUnidade !== 'todas') {
      return empenhosBase.filter(
        (empenho) => empenho.unidadeSaudeId === filtroUnidade,
      )
    }
  ...
  const [modalConfirmacao, setModalConfirmacao] =
    useState<EstadoModalConfirmacao>({
      aberto: false,
    })
  ...
  const abrirFormulario = useCallback(() => {
    setFormulario({
      ...
      dados: {
        unidadeSaudeId:
          filtroUnidade && filtroUnidade !== 'todas' ? filtroUnidade : '', // Pré-selecionar unidade se há filtro
        numeroEmpenho: '',
        valor: '',
        dataEmpenho: new Date().toISOString().split('T')[0],
        observacao: '',
  ...
  const excluirEmpenho = useCallback(
    async (id: string) => {
  ...
  const atualizarCampo = useCallback(
    (campo: keyof EmpenhoForm, valor: string | number | undefined) => {
      setFormulario((prev) => ({
        ...prev,
      ...
      if (campo === 'numeroEmpenho' && typeof valor === 'string') {
        ...
        if (valor.length >= 4) {
          ...
          const validacaoUnico = validarNumeroEmpenhoUnico(
            valor,
            empenhosExibidos,
          )
  ...
  const salvarEmpenhoHandler = useCallback(async () => {
    try {
      ...
      if (
        filtroUnidade &&
        filtroUnidade !== 'todas' &&
        dados.unidadeSaudeId !== filtroUnidade
      ) {
        ...
        <Card className="border-red-200 bg-red-50">
          <CardContent className="p-6">
            ...
            <p className="mb-4 text-red-700">
              O servidor está enfrentando problemas técnicos após múltiplas
              tentativas de conexão. Por favor, tente novamente em alguns
              minutos.
            </p>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
              <div className="text-center">
                ...
                <p className="text-muted-foreground text-sm">Valor Empenhado</p>
              </div>
              <div className="text-center">
              ...
              <div className="text-center">
                ...
                <p className="text-muted-foreground text-sm">Percentual</p>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Progress
                value={estatisticas.percentualEmpenhado}
                className="h-2"
              />
            </div>
          </CardContent>
              ...
              <div className="flex justify-between">
                ...
                {filtroUnidade && filtroUnidade !== 'todas' ? (
                  ...
                  <Badge variant="outline">
                    {estatisticas.unidadesComEmpenho}
                  </Badge>
                )}
              </div>
              <div className="flex justify-between">
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              {filtroUnidade && filtroUnidade !== 'todas' && (
                <Button
                  ...
                  className="hover:bg-secondary/80 text-xs"
                >
            ...
            <Button
              ...
              aria-label="Adicionar novo empenho"
            >
              ...
              {filtroUnidade && filtroUnidade !== 'todas' && (
                <div className="flex items-center gap-2">
                  <Button
                    ...
                    size="sm"
                    onClick={() => navigate(`/unidades/${filtroUnidade}`)}
                    className="hover:bg-secondary/80 h-6 px-2 text-xs"
                  >
                    Filtrado: {nomeUnidadeFiltro}
                    <ExternalLink className="ml-1 h-3 w-3" />
                  </Button>
                    ...
                    <div className="flex-1 space-y-2">
                      ...
                      <div className="text-muted-foreground flex items-center gap-4 text-sm">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <span>
                            {dateUtils.formatarDataUTC(empenho.dataEmpenho)}
                          </span>
                        </div>
                      </div>
        ...
        {formulario.aberto && (
          ...
          >
            <motion.div
              ...
              className="max-h-[90vh] w-full max-w-2xl rounded-lg bg-white shadow-xl"
            >
              <div className="p-6">
                <div className="mb-6 flex items-center justify-between">
                  <h2 className="text-xl font-semibold">Novo Empenho</h2>
                  <Button
                    variant="ghost"
                    size="sm"
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        atualizarCampo('unidadeSaudeId', value)
                      }
                    >
                    ...
                    {filtroUnidade &&
                      filtroUnidade !== 'todas' &&
                      formulario.dados.unidadeSaudeId === filtroUnidade && (
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        atualizarCampo('numeroEmpenho', e.target.value)
                      }
                    ...
                    <p className="text-muted-foreground text-xs">
                      Formato: AAAA + qualquer texto (ex: 2025NE000123,
                      2025NOTA001, etc.)
                    </p>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-muted-foreground text-xs">
                      Valores positivos ou negativos são aceitos (exceto R$
                      0,00)
                    </p>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        atualizarCampo('dataEmpenho', e.target.value)
                      }
      ...
      <AnimatePresence>
        {modalConfirmacao.aberto && (
          <motion.div
            ...
            className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 p-4"
            onClick={(e) =>
              e.target === e.currentTarget && fecharModalConfirmacao()
            }
          >
            <motion.div
              initial={{ scale: 0.95, opacity: 0 }}
            ...
            >
              <div className="p-6">
                ...
                <p className="mb-6 text-gray-700">
                  ...
                  <strong>{modalConfirmacao.numeroEmpenho}</strong>? Esta ação
