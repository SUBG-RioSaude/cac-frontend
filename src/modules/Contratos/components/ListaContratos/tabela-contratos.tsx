...
import { Eye, Edit, ChevronLeft, ChevronRight, MoreHorizontal, Calendar, Building, DollarSign } from 'lucide-react'
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
        ...
        <CardContent className="p-4">
          <div className="flex items-start justify-between mb-3">
            <div className="flex items-center gap-3">
              ...
              />
              <div>
                <h3 className="font-semibold text-sm">{contrato.numeroContrato}</h3>
                {contrato.numeroCCon && (
                  <p className="text-xs text-muted-foreground">CCon: {contrato.numeroCCon}</p>
                )}
              </div>
            </div>
          ...
          <div className="space-y-3">
            <div>
              <p className="font-medium text-sm line-clamp-1">{contrato.contratada.razaoSocial}</p>
              <p className="text-xs text-muted-foreground">{formatarCNPJ(contrato.contratada.cnpj)}</p>
            </div>

            <div className="grid grid-cols-2 gap-3 text-xs">
              <div className="flex items-center gap-1">
                <DollarSign className="h-3 w-3 text-muted-foreground" />
                <span className="font-semibold">{formatarMoeda(contrato.valor)}</span>
              </div>
              <div className="flex items-center gap-1">
            ...
            <div className="flex items-center gap-1 text-xs">
              ...
              <span>{formatarData(contrato.dataInicial)} - {formatarData(contrato.dataFinal)}</span>
            </div>

            <p className="text-xs text-muted-foreground line-clamp-2">{contrato.objeto}</p>
      ...
      <Card className="shadow-2xl border-0 bg-card/50 backdrop-blur">
            ...
            <div className="sm:hidden">
              <div className="flex items-center gap-2">
                <Checkbox
                  ...
                  className={algunsContratosSelecionados ? "data-[state=indeterminate]:bg-primary data-[state=indeterminate]:text-primary-foreground" : ""}
                />
        ...
        <CardContent className="p-0">
          ...
          <div className="hidden lg:block">
            <div className="rounded-lg border bg-background/50 mx-4 sm:mx-6 mb-6 overflow-hidden">
              <Table>
                <TableHeader>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-4 sm:px-6 pb-6">
              ...
              <div className="flex items-center gap-1">
                ...
                {totalPaginas > 3 && (
                  <>
                    ...
                    <Button
                      variant={paginacao.pagina === totalPaginas ? "default" : "outline"}
                      size="sm"
                      onClick={() => setPaginacao({ ...paginacao, pagina: totalPaginas })}
                      className="w-8 h-8 sm:w-9 sm:h-9 p-0"
                    >
                      {totalPaginas}
                    </Button>
