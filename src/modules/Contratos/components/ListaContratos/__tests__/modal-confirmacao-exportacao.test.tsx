...
describe('ModalConfirmacaoExportacao', () => {
  ...
  it('deve renderizar o modal quando isOpen é true', () => {
    ...
    expect(screen.getByText('Você está prestes a exportar todos os contratos')).toBeInTheDocument()
  })
  ...
  it('deve exibir o total de contratos formatado', () => {
    render(<ModalConfirmacaoExportacao {...propsPadrao} totalContratos={1500} />)
  ...
  it('deve exibir mensagem sobre tempo de processamento', () => {
    render(<ModalConfirmacaoExportacao {...propsPadrao} />)
    expect(screen.getByText(/Esta ação pode levar alguns segundos/)).toBeInTheDocument()
  })
  ...
  it('deve aplicar classes CSS corretas para o botão cancelar', () => {
    ...
    expect(botaoCancelar).toHaveClass('inline-flex', 'items-center', 'justify-center')
  })
  ...
  it('deve aplicar classes CSS corretas para o container de informações', () => {
    ...
    const containerInfo = screen.getByText('Total de contratos:').closest('.rounded-lg')
    expect(containerInfo).toHaveClass('rounded-lg', 'bg-muted/50', 'p-4', 'space-y-2')
  })
  ...
  it('deve exibir informações organizadas em linhas', () => {
    ...
    linhasInfo.forEach(linha => {
      ...
      expect(container).toHaveClass('flex', 'items-center', 'justify-between', 'text-sm')
    })
  })

  it('deve aplicar classes CSS corretas para o conteúdo do modal', () => {
    render(<ModalConfirmacaoExportacao {...propsPadrao} />)
    const conteudoModal = screen.getByText('Confirmar Exportação').closest('[role="dialog"]')
    expect(conteudoModal).toBeInTheDocument()
  })

  it('deve formatar diferentes quantidades de contratos', () => {
    const { rerender } = render(<ModalConfirmacaoExportacao {...propsPadrao} totalContratos={1} />)
    expect(screen.getByText('1')).toBeInTheDocument()

    rerender(<ModalConfirmacaoExportacao {...propsPadrao} totalContratos={1000000} />)
  ...
  it('deve aplicar classes CSS corretas para o header do modal', () => {
    ...
    const header = screen.getByText('Confirmar Exportação').closest('.flex.flex-col.gap-2')
