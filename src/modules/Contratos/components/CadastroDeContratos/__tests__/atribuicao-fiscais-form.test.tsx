...
describe('AtribuicaoFiscaisForm', () => {
  ...
  describe('Renderização inicial', () => {
    it('deve renderizar o título da seção de usuários disponíveis', () => {
      ...
      expect(screen.getByText('Usuários Atribuídos ao Contrato')).toBeInTheDocument()
    })

    it('deve exibir o campo de busca', () => {
      setupComponent()
      
      expect(screen.getByPlaceholderText('Buscar por nome ou matrícula...')).toBeInTheDocument()
    })
    ...
    it('deve mostrar mensagem quando não há usuários atribuídos', () => {
      ...
      expect(screen.getByText('Nenhum usuário atribuído ainda')).toBeInTheDocument()
      expect(screen.getByText('Selecione usuários da lista ao lado')).toBeInTheDocument()
    })
  })
  ...
  describe('Funcionalidade de busca', () => {
    it('deve filtrar usuários por nome', async () => {
      ...
      const inputBusca = screen.getByPlaceholderText('Buscar por nome ou matrícula...')
    ...
    it('deve mostrar mensagem quando nenhum usuário é encontrado', async () => {
      ...
      const usuariosVisiveis = screen.getAllByText(/João|Maria|Pedro|Ana|Carlos|Lucia|Roberto/i)
  ...
  describe('Remoção de usuários', () => {
    it('deve remover usuário da lista de atribuídos', async () => {
      ...
      const listaAtribuidos = screen.getByText('Usuários Atribuídos ao Contrato').closest('div')?.parentElement
      if (listaAtribuidos) {
        const usuariosAtribuidos = listaAtribuidos.querySelectorAll('[class*="bg-white"]')
  ...
  describe('Seleção de tipo de usuário', () => {
    ...
    it('deve aplicar cores corretas para cada tipo', async () => {
      ...
      expect(badgeGestor).toBeInTheDocument()
      expect(badgeGestor).toHaveClass('bg-green-100', 'text-green-800', 'border-green-200')
    })
  })
