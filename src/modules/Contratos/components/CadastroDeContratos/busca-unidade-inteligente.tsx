...
import { Search, Building2, MapPin, Phone, Mail, Check, X } from "lucide-react"
import unidadesData from "@/modules/Contratos/data/unidades.json"
import type { UnidadeHospitalar } from "@/modules/Contratos/types/unidades"
import { cn } from "@/lib/utils"

// Convertendo o JSON para o tipo correto
const unidadesHospitalares: UnidadeHospitalar[] = unidadesData as UnidadeHospitalar[]
...
export default function BuscaUnidadeInteligente({
  ...
  // Simula busca com delay para parecer mais realista
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const resultadosFiltrados = unidadesHospitalares.filter((unidade) => {
        const termo = termoBusca.toLowerCase()
        return (
          unidade.nome.toLowerCase().includes(termo) ||
          unidade.sigla.toLowerCase().includes(termo) ||
          unidade.ug.includes(termo) ||
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center space-x-2 mb-1">
                        <h4 className="font-medium text-gray-900 truncate">
                          {destacarTexto(unidade.nome, termoBusca)}
                        </h4>
                        <Badge variant="outline" className="text-xs">
                          {destacarTexto(unidade.sigla, termoBusca)}
                        </Badge>
                      </div>

                      <div className="grid grid-cols-2 gap-2 text-xs text-gray-600">
                        <div>
                          <span className="font-medium">UG:</span> {destacarTexto(unidade.ug, termoBusca)}
                        </div>
                        <div>
                          <span className="font-medium">CNPJ:</span> {destacarTexto(unidade.cnpj, termoBusca)}
                        </div>
      ...
      {mostrarResultados && resultados.length === 0 && termoBusca.length >= 2 && !carregando && (
        <Card className="absolute top-full left-0 right-0 z-50 mt-1 border shadow-lg">
          <CardContent className="p-4 text-center">
            ...
            <p className="text-xs text-gray-500">Tente buscar por UG, sigla, CNPJ ou nome da unidade</p>
          </CardContent>
        </Card>
      )}
    </div>
