 ...
 */
export async function listarEmpenhosPorContrato(
  contratoId: string,
): Promise<Empenho[]> {
 ...
 */
export function validarNumeroEmpenho(numeroEmpenho: string): {
  valido: boolean
  erro?: string
} {
 ...
 */
export function validarValor(valor: number | string): {
  valido: boolean
  erro?: string
} {
  const valorNumerico =
 ...
 */
export function validarDataEmpenho(dataEmpenho: string): {
  valido: boolean
  erro?: string
} {
  if (!dataEmpenho) {
...
export function validarNumeroEmpenhoUnico(
  ...
  const numeroExistente = empenhos.find(
    (emp) => emp.numeroEmpenho === numeroEmpenho && emp.id !== empenhoIdAtual,
  )
...
export function calcularEstatisticas(
  ...
  const percentualEmpenhado =
    valorTotalContrato > 0
      ? (valorTotalEmpenhado / valorTotalContrato) * 100
      : 0

  const unidadesComEmpenho = new Set(empenhos.map((emp) => emp.unidadeSaudeId))
    .size
