  ...
  return useMutation({
    mutationFn: async (payload: CriarContratoData) => {
      ...
      if (
        ...
        throw new Error(
          'É necessário fornecer unidadesResponsaveis ou unidadeDemandanteId/unidadeGestoraId',
        )
      }
      ...
      const payloadAPI = {
        ...
        prazoInicialMeses: calcularPrazoMeses(
          payload.vigenciaInicial,
          payload.vigenciaFinal,
        ),
        valorGlobal:
          typeof payload.valorGlobal === 'string'
            ? parseFloat(
                (payload.valorGlobal as string)
                  .replace(/[^\d,]/g, '')
                  .replace(',', '.'),
              )
            : payload.valorGlobal,
        ...
        unidadesVinculadas: (payload.unidadesVinculadas ?? []).map(
          (unidade) => ({
            ...
            valorAtribuido:
              typeof unidade.valorAtribuido === 'string'
                ? parseFloat(
                    (unidade.valorAtribuido as string)
                      .replace(/[^\d,]/g, '')
                      .replace(',', '.'),
                  )
                : unidade.valorAtribuido,
            vigenciaInicialUnidade: converterDataParaISO(
              unidade.vigenciaInicialUnidade,
            ),
            vigenciaFinalUnidade: converterDataParaISO(
              unidade.vigenciaFinalUnidade,
            ),
  ...
  return useMutation({
    ...
    onMutate: async (data) => {
      ...
      await queryClient.cancelQueries({
        queryKey: contratoKeys.detail(data.id),
      })

      // Snapshot dos dados atuais para rollback
      const previousContrato = queryClient.getQueryData(
        contratoKeys.detail(data.id),
      )

      // Optimistic update
      queryClient.setQueryData(
        contratoKeys.detail(data.id),
        (old: Contrato | undefined) =>
          old
            ? {
                ...old,
                ...data,
