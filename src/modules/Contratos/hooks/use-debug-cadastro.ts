...
export function useDebugCadastro() {
  ...
  const gerarCnpjValido = useCallback(() => {
    ...
    const soma1 = base.reduce(
      (acc, digit, index) => acc + digit * pesos1[index],
      0,
    )
    ...
    const soma2 = baseComDigito1.reduce(
      (acc, digit, index) => acc + digit * pesos2[index],
      0,
    )
  ...
  const gerarDadosAleatorios = useCallback(() => {
    ...
    return {
      ...
      inscricaoEstadual:
        `${String(timestamp).slice(-8)}${Math.floor(Math.random() * 99)}`
          .padStart(11, '0')
          .slice(0, 11),
      inscricaoMunicipal: `${String(timestamp).slice(-6)}${randomSeed}`
        .padStart(10, '0')
        .slice(0, 10),
      ...
      complemento:
        Math.random() > 0.7
          ? `Sala ${Math.floor(Math.random() * 999) + 1}`
          : '',
  ...
  const getMockContrato = useCallback(async (): Promise<DadosContrato> => {
    ...
    const mapearUnidadeSaudeParaHospitalar = (
      unidadeApi: UnidadeSaudeApi,
    ): UnidadeHospitalar => {
      return {
        id: unidadeApi.id,
    ...
    try {
      ...
      if (responseUnidades.dados.length > 0) {
        ...
        if (unidadesDisponiveis.length > 0) {
          ...
          const unidadeDemandante =
            unidadesDisponiveis[
              Math.floor(Math.random() * unidadesDisponiveis.length)
            ]
    ...
    if (!unidadeDemandanteSelecionada || !unidadeGestoraSelecionada) {
      ...
      unidadeDemandanteSelecionada =
        unidadesMock[Math.floor(Math.random() * unidadesMock.length)]
      unidadeGestoraSelecionada =
        unidadesMock[Math.floor(Math.random() * unidadesMock.length)]
    ...
    const formaPagamento = ['Mensal', 'Etapas', 'Outro'][
      Math.floor(Math.random() * 3)
    ] as 'Mensal' | 'Etapas' | 'Outro'
    const formaPagamentoComplemento =
      formaPagamento === 'Outro'
        ? ['A cada entrega', 'Conforme cronograma', 'Após aprovação'][
            Math.floor(Math.random() * 3)
          ]
        : undefined
  ...
  const getMockUnidades = useCallback((): DadosUnidades => {
    ...
    for (let i = 0; i < numUnidades; i++) {
      ...
      const percentual = isUltima
        ? percentualRestante
        : Math.floor(
            Math.random() * (percentualRestante - (numUnidades - i - 1) * 10),
          ) + 10
      ...
      const valor = (
        percentual * Math.floor(Math.random() * 5000 + 1000)
      ...
      unidades.push({
        ...
        unidadeHospitalar: {
          ...
          sigla,
          cnpj: `${Math.floor(Math.random() * 99999999999999)}.000001/0001-${Math.floor(
            Math.random() * 99,
          )
            .toString()
            .padStart(2, '0')}`,
          cep: `${Math.floor(Math.random() * 99999)
            .toString()
  ...
  const getMockAtribuicao = useCallback((): DadosAtribuicao => {
    ...
    const cargosFiscal = [
      ...
      'Técnico em Contratos',
      'Especialista em Fiscalização',
    ]
    const cargosGestor = [
      'Gestor de Contratos',
      'Coordenador',
    ...
    for (let i = 0; i < numFiscais; i++) {
      ...
      const cargo =
        cargosFiscal[Math.floor(Math.random() * cargosFiscal.length)]
    ...
    for (let i = 0; i < numGestores; i++) {
      ...
      const departamento =
        departamentos[Math.floor(Math.random() * departamentos.length)]
      const cargo =
        cargosGestor[Math.floor(Math.random() * cargosGestor.length)]
      // Matrícula única baseada no timestamp
  ...
  const preencherDadosMock = useCallback(
    async (step: number) => {
      if (step === 2) {
